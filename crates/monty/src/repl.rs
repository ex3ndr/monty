    ...
    pub fn call_function(
        ...
        HeapReader::with(
            ...
            |reader, (interns, print)| {
                let mut vm = VM::new(reader, globals, interns, None, print.reborrow());
                {
                    let vm = &mut vm;
                    ...
                    let result =
                        match vm.evaluate_function("MontyRepl::call_function", callable, arg_values) {
                            Ok(value) => Ok(MontyObject::new(value, vm)),
                            Err(e) => Err(e.into_python_exception(&self.interns, |fname| {
